...
| `crossplane-config/` | `XVCluster` XRD, Composition, and ProviderConfigs. |
...
| `participant-claims/` | One `XVCluster` claim per participant pair — the scale lever. |
| `projects/` | ArgoCD `AppProject` definitions. |
| `vcluster-platform/` | vCluster Platform `Project` config and HTTPRoute. |

To add a new participant pair, drop a file into `participant-claims/`

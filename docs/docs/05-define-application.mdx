...
applications.workshop.example.io    True          True      5s
```

`ESTABLISHED=True` means the `Application` CRD is registered; you can now apply XRs.
...
Note the `CombineFromComposite` patch on `backend-deployment` — that's the plumbing that injects the XR's `spec.message` and `spec.color` into the http-echo container's `-text` argument at reconcile time.

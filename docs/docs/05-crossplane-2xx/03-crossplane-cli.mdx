...
# (Guided) Use the Crossplane CLI ⏱️ 23m
...
:::warning Docker required for `render`
[§5.3](#53-preview-a-composition-with-render) runs the function pipeline locally in a container, so you need a working Docker daemon on your laptop. Without Docker, install it first (see [Solo local setup §0.1](../solo-local-setup) for one-line instructions) or skip ahead to [§5.4](#54-trace-a-live-xr-with-beta-trace) — `beta trace` and `beta validate` don't need it.
:::

## 5.1 Before you start ⏱️ 4m
...
You're about to: install the binary, render the module-5 Composition against a sample XR, trace your live `wall-tile` Application, and validate a manifest offline.

## 5.2 Install the CLI ⏱️ 3m
...
## 5.3 Preview a Composition with `render` ⏱️ 7m {#53-preview-a-composition-with-render}
...
## 5.4 Trace a live XR with `beta trace` ⏱️ 5m {#54-trace-a-live-xr-with-beta-trace}

`render` shows what the Composition *would* produce. `beta trace` shows what it *did* produce, against a live XR. List your Applications:
...
## 5.5 Validate manifests offline with `beta validate` ⏱️ 4m
...
Up to now you've only ever seen a Composition's output *after* applying it — you'd `kubectl apply`, wait, then dig through cluster state. The `crossplane` CLI gives you a way to ask "what would this do?" and "what is this doing right now?" without making the cluster guess on your behalf. That's the difference between debugging by trial-and-error and debugging by looking.

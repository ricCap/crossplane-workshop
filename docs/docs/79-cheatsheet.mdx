...
Heredocs are bash/zsh syntax. On **fish** or **PowerShell**, pipe a quoted string instead — same effect, no `<<EOF`:
...
Either way, when a manifest is more than a few lines, save it to a file and `kubectl apply -f path/to/manifest.yaml` — works in every shell, and you can edit and re-apply without scrolling through history.
...
**One picture for the whole loop:**

<ThemedImage
  alt="Users apply either an XR (which triggers a Composition that emits native resources or MRs) or an MR directly (which a Provider Pod watches and reconciles by calling an external API). The two halves of Crossplane meet when a Composition emits an MR."
  sources={{

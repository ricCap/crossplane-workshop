...
You're about to: install one composition function, define an API called `XHello` (the leading `X` is the Crossplane convention — see the [cheatsheet](../cheatsheet#naming-convention-xr-kinds-start-with-x)), write a recipe that turns each `XHello` into one `ConfigMap`, and apply an instance.

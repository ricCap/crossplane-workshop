...
You've now seen most of the components a Crossplane platform is built from. Two new ones land in this module: **Provider** and **ProviderConfig**.

| Component | What it is | Who creates it | Scope |
|---|---|---|---|
| **CRD** | Kubernetes Custom Resource Definition. Extends the API with a new kind. | Crossplane (auto-generated from an XRD); the cluster operator for built-ins | Cluster |
| **XRD** | Composite Resource Definition — your declaration of a new Crossplane API. Compiles into a CRD when applied. | You (the platform author) | Cluster |
| **XR** | Composite Resource — an instance of an XRD that triggers a Composition. | Platform users | Namespaced (v2) |
| **Composition** | The recipe. "When this XR exists, produce these resources." | You (the platform author) | Cluster |
| **Composition function** | Pluggable logic the Composition's pipeline runs to produce desired state (`function-patch-and-transform`, …). | Function package author; you install it | Cluster |
| **MR** | Managed Resource — a Kubernetes representation of an external thing a Provider reconciles. | Crossplane (composed) or you (directly) | Either, depending on the Provider |
...
| **ProviderConfig** ← *new this module* | Per-Provider runtime config (credentials, target endpoint). Namespaced flavor. | You | Namespaced (v2) |
| **ClusterProviderConfig** ← *new this module* | Per-Provider runtime config — cluster-scoped flavor, shared across namespaces. | You | Cluster |

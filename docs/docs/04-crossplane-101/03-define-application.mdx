...
Same XRD-Composition-XR shape as [module 4](./first-composition), now with six composed resources instead of one:

<ThemedImage
  alt="One Application XR fans out into six native Kubernetes resources: a ConfigMap, Deployment, and Service for the frontend, and the same three for the backend."
  sources={{
...
The XRD and Composition sit upstream (same yellow/platform boxes from module 4, omitted here to avoid repetition). The XR's `spec.message` and `spec.color` flow into one of the six composed resources — the **backend ConfigMap** — via a `CombineFromComposite` patch; everything else in the Composition is static template content. The frontend and backend Deployments each `volumeMount` their respective ConfigMap (plain Kubernetes wiring, not Crossplane-specific).

...
The Composition is the recipe. It creates five resources when an `Application` exists. The picture, then the table, then the YAML.
...
flowchart TB
  ...
  subgraph composed["Composed resources (default namespace)"]
    ...
    bd["Deployment<br/>backend / http-echo<br/>-text=…patched…"]
    bs["Service<br/>backend :5678"]
  end

  xr == "triggers" ==> comp ==> fn
  fn ==> fcm & fd & fs & bd & bs
  xr -. "CombineFromComposite<br/>spec.message + spec.color<br/>→ args[1]" .-> bd
  fd -. "volumeMounts" .-> fcm
```

The dashed arrow is the patch — the only place the XR's `message` and `color` fields actually flow into one of the composed resources. Everything else in the Composition is static template content. The other dashed arrow is a normal Kubernetes reference (the frontend Deployment mounts the frontend ConfigMap by name) — that's plain Kubernetes plumbing, nothing Crossplane-specific.

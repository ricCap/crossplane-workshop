...
The composition function from module 4 (`function-patch-and-transform`) is already installed. No new core-object types this module — but the Composition itself uses three function-pipeline mechanics worth naming up front.

### Composition mechanics introduced this module

| Mechanic | What it does | The variant(s) you'll meet here |
|---|---|---|
| **Multi-resource `resources[]`** | A single Composition can emit any number of composed resources from one function step. Each entry has a `name` (Crossplane-internal label), a `base` (the resource template), optional `patches`, and `readinessChecks` | Five `resources[]` entries — frontend ConfigMap, frontend Deployment + Service, backend Deployment + Service |
| **`patches`** | Field-level wiring between the XR's spec and the composed resource's spec. Each patch declares `fromFieldPath` and `toFieldPath`, and a `type` that says how the value is shaped | `FromCompositeFieldPath` (you saw this in module 4 — copy XR field into composed field). New here: `CombineFromComposite`, which formats multiple XR fields into one composed-resource field via a `printf`-style template |
| **`readinessChecks`** | Tells `function-patch-and-transform` when to mark each composed resource Ready. Required — without it, P&T may hold a resource Pending forever if its kind doesn't publish a Ready condition | `type: None` (you used this in module 4 — "Ready as soon as observed", correct for ConfigMaps and Services). New here: `type: MatchCondition`, which waits for a specific condition like `Available=True` on a Deployment |
...
The Composition is the recipe. It creates five resources when an `Application` exists. Read the table below first; it's a map of the YAML you'll apply.
...
| `backend-deployment` | `Deployment` | Runs `hashicorp/http-echo`, returning a JSON blob on port 5678 | **one** — `CombineFromComposite` writes the XR's `spec.message` and `spec.color` into the container's `-text` argument as a single JSON-formatted string | `type: MatchCondition` waiting for `Available=True` |
| `backend-service` | `Service` | Exposes the backend on port 5678. The HTTPRoute resolves `/team/<pair>/api/` to this Service | none | `type: None` |

The interesting wiring is the patch on `backend-deployment`. The XR ships two scalar fields:
...
`http-echo`'s `-text` argument expects a single string — the literal HTTP response body it'll echo back. The frontend's JS expects that body to be JSON shaped like `{"message":"…","color":"…"}`. `CombineFromComposite` glues the two together with a `printf`-style template:
...
`toFieldPath` is the actual path inside the naked `Deployment` (compare to module 4's modules-of-`Object`-MR shape, which would have prefixed it with `spec.forProvider.manifest.…`). At reconcile time, Crossplane substitutes the variables into the template and writes the result to that path on the live Deployment.

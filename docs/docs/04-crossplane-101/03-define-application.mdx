...
This is the module where the workshop clicks together. You met the XRD-Composition-XR shape in [module 4](./first-composition) on a tiny `Hello` example — one XR fanned out into one ConfigMap. This module uses the same shape on something useful: a high-level `Application` kind that fans out into a frontend, a backend, and the ConfigMap that wires them together. (No `X` prefix — see the [cheatsheet](../cheatsheet#naming-convention-no-x-prefix-in-v2) for why the old `XApplication`-style convention is gone in v2.)

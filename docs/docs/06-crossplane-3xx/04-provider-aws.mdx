...
- **Compose around it.** Wrap `Bucket` (and its sibling `BucketAcl`, `BucketPolicy`, `BucketServerSideEncryptionConfiguration`) in an XR like `Bucket.workshop.example.io` so platform users get an opinionated, encrypted-by-default bucket from a single line of YAML. (The custom group disambiguates from the AWS `Bucket.s3.aws.upbound.io` MR — that's how you avoid kind collisions in v2 without the old `X` prefix; see the [cheatsheet](../cheatsheet#naming-convention-no-x-prefix-in-v2).) That's the same pattern you used in 101 module 5 to wrap `Release.helm.m.crossplane.io`.

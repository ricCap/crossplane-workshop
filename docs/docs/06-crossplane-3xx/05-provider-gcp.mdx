...
- **Compose around it.** Wrap `Bucket` (and its siblings `BucketIAMPolicy`, `BucketObject`) in an XR like `Bucket.workshop.example.io` so platform users get an encrypted, retention-policied, uniform-access bucket from a single line of YAML. (The custom group disambiguates from the GCP `Bucket.storage.gcp.upbound.io` MR — see the [cheatsheet](../cheatsheet#naming-convention-no-x-prefix-in-v2) for why v2 dropped the old `X` prefix.)

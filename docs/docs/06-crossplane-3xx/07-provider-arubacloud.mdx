---
sidebar_position: 7
title: provider-arubacloud against your platform's Aruba project
---

import PairId from '@site/src/components/PairId';
import ValidateCheck from '@site/src/components/ValidateCheck';

# provider-arubacloud against your platform's Aruba project ⏱️ 25m
...
provider-arubacloud   True        True      ghcr.io/riccap/provider-arubacloud:v0.2.0-workshop
```
...
The headline resource. You'll create one **DBaaS** managed resource (a real MySQL instance) using values your instructor pinned on a slide for the shared workshop network.

### 1. Get the shared values

Your instructor has shared four strings:

- **`<workshop-project-id>`** — Aruba project ID the workshop runs in (24-character hex string)
- **`<vpc-uri>`** — URI of the shared workshop VPC
- **`<subnet-uri>`** — URI of a subnet inside that VPC
- **`<security-group-uri>`** — URI of a security group attached to the VPC

Have these four values handy; you'll paste them in §7.3.2.
...
- **Compose a least-privilege MySQL stack.** This module created the DBaaS instance and connected as admin — fine for a workshop, never for production. Real apps need a non-admin login: a `Database.database.arubacloud.crossplane.io` (logical schema) + `DBaaSUser` (non-admin login) + `DatabaseGrant` (the `GRANT … TO …` row that wires them together). Wrap the four MRs in an XR like `XAppDatabase` so platform users get an opinionated "give me a DB for app X" workflow from one line of YAML — same shape as the [`XApplication`](../crossplane-101/define-application) you wrote in 101 module 4.
- [`provider-arubacloud`](https://github.com/ricCap/provider-arubacloud) — provider source, full Kind catalogue, version index. The workshop pins `v0.2.0-workshop`.

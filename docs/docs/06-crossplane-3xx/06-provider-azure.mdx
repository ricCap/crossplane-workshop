---
sidebar_position: 6
title: provider-azure against your own Azure subscription
---

import PairId from '@site/src/components/PairId';
import ValidateCheck from '@site/src/components/ValidateCheck';

# provider-azure against your own Azure subscription ⏱️ 35m
...
- **Compose around it.** Wrap `Account` (and its siblings `Container`, `Blob`, `ManagementPolicy`) in an XR like `XBucket` so platform users get an opinionated, encrypted, lifecycle-managed account from one line of YAML — same shape as the [`XApplication`](../crossplane-101/define-application) you wrote in 101 module 4.

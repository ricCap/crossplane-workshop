...
[Crossplane recommends](https://docs.crossplane.io/latest/composition/composite-resource-definitions/) starting **the XR kind** with `X` so it cannot collide with downstream API kinds the Composition might emit: `Application` is an ArgoCD kind, `Database` ships in several providers, `Bucket` is an S3 MR. Naming your XR `XApplication` keeps yours and theirs unambiguous in `kubectl get`.
...
- The XR instance's `metadata.name` — `wall-tile`, `hello-world`, whatever you pick. No X prefix.

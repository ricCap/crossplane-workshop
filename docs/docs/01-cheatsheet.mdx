...
Crossplane turns any external API (cloud provider, SaaS, another Kubernetes cluster) into Kubernetes resources. You define what you want; a **Provider** watches those resources and makes reality match.

On top of that, you can compose your own abstractions: `Application`, `Database`, `Environment`. You define the schema (**XRD**) and the recipe (**Composition**); users create instances (**XR**) and Crossplane fans them out into managed resources.

### Core objects

| Object | What it is | Scope |
|---|---|---|
| **CRD** | Kubernetes Custom Resource Definition. Extends the API server with a new kind. Crossplane auto-generates one when you apply an XRD | Cluster |
| **Composite Resource Definition (XRD)** | Declares the schema of a new composite kind, e.g. `Application` with `frontend` + `backend` fields. Compiles into a CRD when applied | Cluster |
...
| **Composition function** | Pluggable logic the Composition's pipeline runs to produce desired state — `function-patch-and-transform`, `function-go-templating`, `function-kcl`, `function-python`, etc. | Cluster |
| **Composite Resource (XR)** | An instance of an XRD. Created by the user; Crossplane composes it into the resources its Composition declares | Namespaced (v2) |
| **Provider** | A package that teaches Crossplane to manage an external API (AWS, GCP, Kubernetes, Helm, …) | Cluster |
| **ProviderConfig** | Per-Provider runtime config — credentials, target endpoint. v2 namespaced flavor: one per namespace, isolated from other tenants | Namespaced (v2) |
| **ClusterProviderConfig** | Per-Provider runtime config — v2 cluster-scoped flavor, shared across namespaces. The "default" config for single-tenant clusters | Cluster |
| **Managed Resource (MR)** | A Kubernetes representation of an external thing the provider reconciles by calling its API (e.g. `Release.helm.m.crossplane.io`, `Bucket.s3.aws.upbound.io`) | Either, depending on the Provider |

...
| **ProviderConfig** | Per-provider configuration — credentials, default region, etc. Usually one per provider per environment | Cluster |
| **Managed Resource (MR)** | A Kubernetes representation of an external thing. The provider reconciles the external API to match `spec` | Cluster (most) |
| **Composite Resource Definition (XRD)** | Declares the schema of a new composite kind, e.g. `Application` with `frontend` + `backend` fields | Cluster |
| **Composition** | The recipe. "When someone creates an `Application`, here are the managed resources to create" | Cluster |
| **Composite Resource (XR)** | An instance of an XRD. Created by the user; Crossplane composes it into managed resources | Namespaced (v2) |
| **Function** | Pluggable composition logic. `function-patch-and-transform`, `function-kcl`, `function-go-templating`, etc. | Cluster |

...
The XR kind is **prefixed with `X`** by convention — `XApplication`, `XDatabase`, `XHello`. The prefix exists so the kind cannot collide with downstream API kinds the Composition might emit: `Application` is an ArgoCD kind, `Database` ships in several providers, `Bucket` is an S3 MR. Naming your XR `XApplication` keeps yours and theirs unambiguous in `kubectl get`.

The XRD's `metadata.name` is the lowercase plural of the XR kind plus the API group:
...
spec:
  ...
  names:
    kind: XApplication                       # the XR kind — starts with X

...
## 6.3 Genera una credenziale ⏱️ 9m
...
## 6.5 Applica un ProviderConfig e crea uno Storage Account ⏱️ 9m
...
Un file YAML nel tuo cluster ha creato uno storage account vero nella tua subscription Azure vera. Nessun click nel portal, nessun `az storage account create`, nessun Terraform — `kubectl apply` e Azure ti ha consegnato un endpoint storage con nome pubblico. Se qualcuno cambia quell'account dal portal, Crossplane lo riconcilierà tornando al tuo YAML. Il tuo cluster è ora la fonte di verità per un pezzo di infrastruttura cloud che vive in Azure.

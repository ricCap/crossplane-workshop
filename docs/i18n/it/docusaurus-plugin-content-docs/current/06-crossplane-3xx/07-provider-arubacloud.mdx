---
sidebar_position: 7
title: provider-arubacloud sul progetto Aruba della tua piattaforma
translation_source_commit: be8a5252bf8807fe79ff2d72873477404a8d409f
---

import PairId from '@site/src/components/PairId';
import ValidateCheck from '@site/src/components/ValidateCheck';

# provider-arubacloud sul progetto Aruba della tua piattaforma ⏱️ 25m
...
La risorsa di punta. Creerai una managed resource **DBaaS** (un'istanza MySQL reale) usando i valori che il tuo istruttore ha pinnato su una slide per la rete condivisa del workshop.

### 1. Ottieni i valori condivisi

Il tuo istruttore ha condiviso quattro stringhe:

- **`<workshop-project-id>`** — ID del progetto Aruba in cui gira il workshop (stringa esadecimale di 24 caratteri)
- **`<vpc-uri>`** — URI della VPC condivisa del workshop
- **`<subnet-uri>`** — URI di una subnet dentro quella VPC
- **`<security-group-uri>`** — URI di un security group attaccato alla VPC

Tieni a portata questi quattro valori; li incollerai in §7.3.2.
...
- [`provider-arubacloud`](https://github.com/ricCap/provider-arubacloud) — sorgente del provider, catalogo completo dei Kind, indice delle versioni. Il workshop pinna `v0.2.0-workshop`.

...
- **Componici sopra.** Avvolgi `Bucket` (e i suoi parenti `BucketAcl`, `BucketPolicy`, `BucketServerSideEncryptionConfiguration`) in un XR come `Bucket.workshop.example.io` così gli utenti della piattaforma ottengono un bucket opinionated, cifrato di default, da una sola riga di YAML. (Il group custom disambigua dall'MR AWS `Bucket.s3.aws.upbound.io` — è così che si evitano collisioni di kind in v2 senza il vecchio prefisso `X`; vedi il [cheatsheet](../cheatsheet#naming-convention-no-x-prefix-in-v2).) È lo stesso pattern che hai usato nel modulo 5 del 101 per avvolgere `Release.helm.m.crossplane.io`.

...
- **Componici sopra.** Avvolgi `Bucket` (e i suoi parenti `BucketIAMPolicy`, `BucketObject`) in un XR come `Bucket.workshop.example.io` così gli utenti della piattaforma ottengono un bucket cifrato, con retention policy, ad accesso uniforme, da una sola riga di YAML. (Il group custom disambigua dall'MR GCP `Bucket.storage.gcp.upbound.io` — vedi il [cheatsheet](../cheatsheet#naming-convention-no-x-prefix-in-v2) per il motivo per cui v2 ha tolto il vecchio prefisso `X`.)

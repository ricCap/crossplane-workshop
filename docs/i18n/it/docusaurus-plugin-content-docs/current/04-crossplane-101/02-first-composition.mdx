...
flowchart TB
  user(["You"])
  xrd["XRD<br/>xhellos.workshop.example.io"]
  crd["CRD<br/>xhellos.workshop.example.io<br/>(generated)"]
  comp["Composition<br/>xhellos.workshop.example.io"]
  fn["function-patch-and-transform"]
  xr["XHello XR<br/>hello-world"]
...
Stai per: installare una composition function, definire un'API chiamata `XHello` (la `X` iniziale è la convenzione Crossplane — vedi il [cheatsheet](../cheatsheet#naming-convention-xr-kinds-start-with-x)), scrivere una ricetta che trasforma ogni `XHello` in una `ConfigMap`, e applicare un'istanza.

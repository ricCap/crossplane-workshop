...
### Un Provider, in due immagini

Un Provider ha due fasi che vale la pena vedere separate — **install-time** (il platform engineer lo cabla) e **runtime** (l'utente applica una MR; il Pod del Provider la riconcilia).

#### Install-time — cosa applica il platform engineer

<ThemedImage
  alt="Il platform engineer applica un pacchetto Provider; Crossplane core lo installa avviando un Pod Provider long-running."
  sources={{
...
Il package del Provider è ciò che fa nascere il Pod long-running. I due ProviderConfig (cluster-scoped e namespaced) dicono a quel Pod *come autenticarsi* verso l'API esterna — non innescano riconciliazione da soli.

#### Runtime — cosa succede quando un utente applica una MR

<ThemedImage
  alt="Un utente applica una Managed Resource Release. Il Pod del Provider la osserva, scarica la chart da un registry OCI e applica le risorse Kubernetes risultanti all'apiserver del cluster."
  sources={{
...
Il Pod osserva le MR dei suoi kind — `Release.helm.m.crossplane.io` qui — e traduce ognuna in chiamate verso un'API esterna. Per `provider-helm` quell'"API" è l'SDK Helm che scarica chart e le applica all'apiserver dello stesso cluster. Per `provider-aws-s3` sarebbe l'SDK AWS; la forma è identica.
...
## 7.2 Installa `provider-helm` ⏱️ 5m
...
## 7.3 Configura il provider ⏱️ 5m
...
## 7.4 Installa una Helm chart attraverso Crossplane ⏱️ 7m
...
Quando la tile diventa verde, Crossplane ha installato una vera Helm chart attraverso un Provider. Stesso contratto di lifecycle di un XR `Hello` o di un XR `Application` — `kubectl delete release podinfo -n workshop-helm` e la chart se ne va con lui.

## 7.5 Cosa è appena successo

Finora Crossplane sapeva solo creare cose *dentro* il tuo cluster. Installando un Provider, gli hai insegnato ad andare *fuori* — in questo caso, a eseguire un'installazione Helm per conto tuo. Dal tuo lato sembra ancora la stessa cosa: hai scritto un file YAML, hai eseguito `kubectl apply` ed è apparsa una cosa. La novità è che Crossplane non sta più solo cucendo insieme risorse Kubernetes; ora è un operatore pratico di uno strumento esterno, con lo stesso comportamento di reconcile-finché-Ready a cui ti sei già abituato.

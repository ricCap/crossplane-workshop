...
# Definisci un'Application ⏱️ 35m
...
## 5.2 Costruisci l'API Application ⏱️ 30m
...
Stessa forma XRD-Composition-XR del [modulo 4](./first-composition), ora con sei risorse composte invece di una:

<ThemedImage
  alt="Un Application XR si espande in sei risorse native Kubernetes: una ConfigMap, un Deployment e un Service per il frontend, e gli stessi tre per il backend."
  sources={{
    light: useBaseUrl('/diagrams/define-application-fanout-light.svg'),
    dark: useBaseUrl('/diagrams/define-application-fanout-dark.svg'),
  }}
  style={{ maxWidth: '100%', height: 'auto' }}
/>

L'XRD e la Composition stanno a monte (gli stessi box gialli/platform del modulo 4, qui omessi per non ripetere). I campi `spec.message` e `spec.color` dell'XR fluiscono in una delle sei risorse composte — la **ConfigMap del backend** — tramite una patch `CombineFromComposite`; tutto il resto nella Composition è contenuto template statico. Frontend e backend Deployment montano (`volumeMount`) ciascuno la propria ConfigMap (semplice wiring Kubernetes, non specifico di Crossplane).

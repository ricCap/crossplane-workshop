...
translation_source_commit: dc7fee841463a55b25c4436dab1f03a0a33e9d34
---

import PairId from '@site/src/components/PairId';

# (Guidato) Usa la CLI Crossplane ⏱️ 23m
...
:::warning Docker richiesto per `render`
[§5.3](#53-preview-a-composition-with-render) esegue la pipeline di function localmente in un container, quindi ti serve un daemon Docker funzionante sul tuo laptop. Senza Docker, installalo prima (vedi [Setup locale solo §0.1](../solo-local-setup) per istruzioni one-line) o salta avanti a [§5.4](#54-trace-a-live-xr-with-beta-trace) — `beta trace` e `beta validate` non ne hanno bisogno.
:::

## 5.1 Prima di iniziare ⏱️ 4m
...
Stai per: installare il binario, fare il render della Composition del modulo 5 contro un XR di esempio, fare il trace della tua Application `wall-tile` live, e validare un manifest offline.

## 5.2 Installa la CLI ⏱️ 3m
...
## 5.3 Anteprima di una Composition con `render` ⏱️ 7m {#53-preview-a-composition-with-render}
...
```bash
kubectl get xrd applications.workshop.example.io -o yaml > xrd.yaml
kubectl get composition applications.workshop.example.io -o yaml > composition.yaml
```
...
## 5.4 Trace di un XR live con `beta trace` ⏱️ 5m {#54-trace-a-live-xr-with-beta-trace}

`render` mostra cosa la Composition *produrrebbe*. `beta trace` mostra cosa ha *effettivamente prodotto*, contro un XR live. Elenca le tue Application:

```bash
kubectl get applications.workshop.example.io -A
```

Poi fai il trace di quella che hai applicato nel modulo 5:

```bash
crossplane beta trace application.workshop.example.io/wall-tile -n default
```
...
Application/wall-tile (default)               True     True    Available
...
```bash
crossplane beta trace application.workshop.example.io/wall-tile -n default -o wide
```
...
## 5.5 Valida manifest offline con `beta validate` ⏱️ 4m
...
```
[✓] workshop.example.io/v1alpha1, Kind=Application, render-demo validated successfully
...
```
[x] schema validation error workshop.example.io/v1alpha1, Kind=Application, render-demo : spec.message: Required value
...
Fino a ora hai visto l'output di una Composition solo *dopo* averla applicata — facevi `kubectl apply`, aspettavi e poi rovistavi nello stato del cluster. La CLI `crossplane` ti dà un modo per chiedere "cosa farebbe?" e "cosa sta facendo adesso?" senza costringere il cluster a indovinare per te. È la differenza tra debuggare per tentativi ed errori e debuggare guardando.

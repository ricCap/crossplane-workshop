...
- **Restore per-pair CPU/memory/ephemeral-storage caps** once Argo CD ships a release vendoring `k8s.io/kubectl` ≥ `v0.36.0`. Tracked upstream at [argoproj/argo-cd#26529](https://github.com/argoproj/argo-cd/issues/26529) (root cause [kubernetes/kubernetes#136533](https://github.com/kubernetes/kubernetes/issues/136533), fix [#136534](https://github.com/kubernetes/kubernetes/pull/136534) shipped in kubectl `v0.36.0`). The `XDeveloperEnvironment` Composition currently emits a stripped-down `ResourceQuota` (object counts + PVC size only) and no `LimitRange`, because kubectl `v0.34.0`'s `populatePodInfo` panics with `assignment to entry in nil map` when the application-controller scans a namespace where a `LimitRange` injects default resources onto pods that ship without explicit `resources`. Revert [gitops/crossplane-config/composition.yaml](gitops/crossplane-config/composition.yaml) and the inline note in [gitops/crossplane-config/xrd.yaml](gitops/crossplane-config/xrd.yaml) when the fix is released.
